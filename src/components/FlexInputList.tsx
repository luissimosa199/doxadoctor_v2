...
const InputList: FunctionComponent<InputListProps> = ({
  ...
  const addInput = (valueToAdd?: string) => {
    ...
    const inputValue =
      type === "tag" ? trimmedValue.toLocaleLowerCase() : trimmedValue;
    ...
    if (type === "tag") {
      ...
      if (
        (inputList as InputItem[]).some((item) => item.value === inputValue)
      ) {
        setInputText("");
        return;
      }
          ...
          <button
            ...
            onClick={(event) => {
              ...
              if (type === "tag") {
                ...
                setInputList((prevList: InputItem[]) =>
                  prevList.filter(
                    (input) =>
                      input.value !== (item as unknown as InputItem).value
                  )
                );
              }
            }}
